...
describe("extractCharacterLines", () => {
  ...
  it("returns lines containing {ref:SlotName}", () => {
    const template = "{assign:MainTank} Rend\nRef line: {ref:MainTank}\nOther line";
    expect(extractCharacterLines(template, ["MainTank"])).toEqual([
  ...
  it("matches case-insensitively", () => {
    ...
    expect(extractCharacterLines(template, ["maintank"])).toEqual(["Tank: {assign:MAINTANK}"]);
  });
  ...
  it("matches slots with ref modifier {ref:SlotName:nocolor}", () => {
    ...
    expect(extractCharacterLines(template, ["MainTank"])).toEqual(["Ref: {ref:MainTank:nocolor}"]);
  });
  ...
  it("only returns lines containing the specified slots", () => {
    ...
    expect(extractCharacterLines(template, ["MainTank"])).toEqual(["MT: {assign:MainTank}"]);
  });

  it("returns a line containing any of multiple specified slots", () => {
    const template = "{assign:MainTank} and {assign:OffTank} on same line\nUnrelated line";
  ...
  it("does not return lines with similar but non-matching slot names", () => {
    ...
    expect(extractCharacterLines(template, ["MainTank"])).toEqual(["OT: {assign:MainTank}"]);
  });
